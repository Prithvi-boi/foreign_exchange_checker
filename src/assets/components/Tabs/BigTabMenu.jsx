        ...
        <>
            <div className='relative border-b-2 border-zinc-800 h-full flex items-center gap-7 px-4'>
                {TabOptions.map((option, key) => {
                    ...
                    {option}
                    <p onClick={(e)=>{e.stopPropagation()}} className={`bg-lime-900 text-[#CEF739] text-sm rounded-full ${option == 'FAVORITES' || option == 'LOGS' ? 'w-6 h-6 ' : '' } grid place-items-center`}>{option == 'FAVORITES' ? favCount : ''}{option == 'LOGS' ?  logCount : '' }</p>
                    </button>
                })}
                <div style={{ width: btnWidth, left: btnOffset-5 }} className='rounded-2xl transition-all duration-300 absolute h-1 w-17 top-full bg-lime-400'></div>
            </div>
        </>
    )
}
